...
- When a row has an alias but no mapped purchase value, scan for purchase from NEIGHBOR rows using this priority: (1) scan DOWNWARD first (subsequent rows in the same item block), then (2) scan UPWARD (previous rows). Scanning downward first prevents accidentally picking up purchase values from a different product item that may appear above. This is particularly important in tables with interleaved "Rated Current" rows or continuation rows.
- In split-row layouts where Camelot separates a product into alternating `technical/current row` and `Cat.No row` records, do not use a fixed neighbor direction for purchase salvage. Infer the local orientation per mapped block (price-above-alias vs price-below-alias) from repeating row patterns, then salvage using that orientation first. This prevents rows like `4086 62` from incorrectly taking the next item's MRP (`15086`) instead of their own (`14163`).
...
- In dual-role cells where a Cat.No stands alone on one row and the next row contains `description text\nMRP` (e.g. `"USB Charger - Power \n13318"`), `leading_alias_from_text` must NOT scan past the first alphabetic description-prose line to return the trailing MRP as an alias. When the first non-empty line fails alias extraction AND starts with `^[A-Za-z]{2,}\s` (multi-word description prose), bail out immediately — trailing numeric lines are embedded prices, not Cat.Nos.
